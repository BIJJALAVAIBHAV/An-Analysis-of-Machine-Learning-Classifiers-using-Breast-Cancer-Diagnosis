...
class MyAlgorithms:
    ...
    def startPreprocess(self, path):
        bcdf = pd.read_csv(path)
        print("Data path ", bcdf.dtypes)
    ...
    def modelExecutions(self, path):
        ...
        print(
            'Mean accuracy is is: {} which is poorer than model accuracy without \ndimensional reduction.'.format(
                results_feature_union.mean()))
